...
// src/models/user-cars-schema.ts
...
var authRegister = async (req, res) => {
  ...
  try {
    ...
    if (userExists)
      return res.status(404).json({ msg: "Usu\xE1rio j\xE1 registrado" });
    ...
    createdUser.save((error, user) => {
      ...
      return res.status(201).json({ msg: `${name}, sua conta foi criada com sucesso!` });
    });
  } catch (error) {
    console.log(error);
  ...
  try {
    ...
    if (!checkPassword)
      return res.status(400).json({
        msg: "O email ou senha inseridos s\xE3o inv\xE1lidos. Por favor, verifique suas credenciais e tente novamente"
      });
    if (!user)
      return res.status(404).json({ msg: "Usu\xE1rio n\xE3o encontrado!" });
    const acessToken = import_jsonwebtoken.default.sign({ id: user._id, email: user.email }, SECRET, {
      expiresIn: "17m"
    });
    const refreshToken2 = import_jsonwebtoken.default.sign({ id: user._id, email: user.email }, SECRET, {
      expiresIn: "17h"
    });
    return res.status(200).json({
      msg: "Autentica\xE7\xE3o realizada com sucesso!",
      userId: user._id,
      acessToken,
...
// src/validators/verify-token.ts
var import_jsonwebtoken2 = __toESM(require("jsonwebtoken"), 1);
function verifyTokenFn(refreshToken2) {
  try {
    const { id: userId } = import_jsonwebtoken2.default.verify(refreshToken2, SECRET);
    return userId;
  } catch (error) {
    return null;
  }
...
// src/controllers/refresh-token-controller.ts
...
var refreshToken = async (req, res) => {
  ...
  if (!authRefreshToken)
    return res.status(401).json({ msg: "Refresh Token n\xE3o informado!" });
  try {
    const userId = verifyTokenFn(authRefreshToken);
    if (!userId)
      return res.status(401).json({ msg: "Refresh Token inv\xE1lido" });
    const user = await User.findById(userId);
    if (!user)
      return res.status(404).json({ msg: "Usu\xE1rio n\xE3o encontrado" });
    const newToken = import_jsonwebtoken3.default.sign({ id: user._id, email: user.email }, SECRET, {
      expiresIn: "17m"
    });
    const newRefreshToken = import_jsonwebtoken3.default.sign(
...
// src/middlewares/check-token.ts
var checkToken = async (req, res, next) => {
  const authHeader = req.headers["authorization"];
  const accessToken = authHeader && authHeader.split(" ")[1];
  if (!accessToken) {
    return res.status(401).json({ msg: "Token n\xE3o fornecido" });
  }
  ...
  res.status(401).json({ msg: "Token inv\xE1lido" });
};
